...
class TwitchUnofficialChattersParserTest {
    ...
    fun `preserves chatter groups and deduplicates users`() {
        ...
        assertEquals(listOf("owner", "mod", "vip", "bot", "viewer"), users.map { it.login })
        assertEquals(listOf("1", "3", "2", "5", "4"), users.map { it.id })
        assertEquals(
            listOf(
    ...
    @Test
    fun `moderator bucket wins over vip for duplicate login`() {
        val body = """
            {
              "data": {
                "user": {
                  "channel": {
                    "chatters": {
                      "broadcasters": [],
                      "staff": [],
                      "vips": [{"id":"2","login":"both"}],
                      "moderators": [{"id":"2","login":"both"}],
                      "chatbots": [],
                      "viewers": []
                    }
                  }
        ...
        val user = TwitchUnofficialChattersParser.parse(body).single()

        assertEquals(TwitchUnofficialChatterGroup.MODERATOR, user.group)
    }
    ...
    @Test(expected = TwitchUnofficialChattersUnavailableException::class)
    fun `empty community result is unavailable instead of authoritative`() {
        requireAuthoritativeCommunityChatters(emptyList())
    }
