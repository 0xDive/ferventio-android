...
def template_parts(value: str) -> list[tuple[str, bool]]:
    ...
    while i < len(value):
        ...
        if value[i + 1] == "{":
            ...
            j = i + 2
            while j < len(value) and IDENT_CHAR_RE.fullmatch(value[j]):
                j += 1
            end = j
        if end is None:
            i += 1
...
def format_template(value: str) -> str:
...
def write_xml(path: Path, values: list[str], translated: dict[str, str] | None) -> None:
    ...
    for value in values:
        ...
        expected_args = argument_count(value)
        if expected_args:
            if argument_count(resolved) != expected_args:
                raise SystemExit(f"placeholder count mismatch for {value!r}: {resolved!r}")
            resolved = format_template(resolved)
