package io.ferventio.shared.ui.theme
...
@Composable
fun FerventioTheme(
    themeMode: FerventioThemeMode = FerventioThemeMode.DARK,
    ...
    MaterialTheme(
        colorScheme = ferventioColorScheme(themeMode),
...
internal fun ferventioColorScheme(themeMode: FerventioThemeMode): ColorScheme = when (themeMode) {
    FerventioThemeMode.LIGHT -> LightColors
    FerventioThemeMode.DARK -> DarkColors
    FerventioThemeMode.AMOLED -> AmoledColors
}
...
private fun TextUnit.scaled(scale: Float): TextUnit =
    if (this != TextUnit.Unspecified) value.times(scale).sp else this
