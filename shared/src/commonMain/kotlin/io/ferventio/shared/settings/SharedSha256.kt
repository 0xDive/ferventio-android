...
internal fun sha256Hex(value: String): String = Sha256.digest(value.encodeToByteArray())
    ...
    fun digest(input: ByteArray): ByteArray {
        ...
        while (offset < padded.size) {
            ...
            for (index in 0 until 64) {
                val sum1 = h +
                    (e.rotateRight(6) xor e.rotateRight(11) xor e.rotateRight(25)) +
        ...
        return ByteArray(32).also { output ->
            hash.forEachIndexed { index, word ->
                val base = index * 4
                output[base] = (word ushr 24).toByte()
                output[base + 1] = (word ushr 16).toByte()
                output[base + 2] = (word ushr 8).toByte()
                output[base + 3] = word.toByte()
            }
        }
    }
}
